...
void compare_pairs(int size, square_list squares) {
  initialize(size);
  square_list sq_ls;
  for (sq_ls = squares; sq_ls != NULL; sq_ls = sq_ls->next) {
    // iterate over the list of squares and comare each square to istelf
    compare_permutations(sq_ls->square, sq_ls->square);
  }
}
